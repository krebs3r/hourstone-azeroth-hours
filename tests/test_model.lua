local M,L=H.M,H.L
assert(M.Format(1125000,"hours")== (H.de and "312,5 Std." or "312.5 hr"))
assert(M.Format(1125000,"combined")== (H.de and "13 T. 0 Std. 30 Min." or "13 d 0 hr 30 min"))
assert(M.Format(nil)==L.unavailable and M.Format(-1)==L.unavailable)
assert(M.Format(0,"combined")== (H.de and "0 T. 0 Std. 0 Min." or "0 d 0 hr 0 min"))
assert(not M.Number(0/0) and not M.Number(math.huge))
local db=M.Init({settings={scale=99,format="broken",position={x="5",y="-8"}}})
assert(db.version==1 and db.settings.scale==1.3 and db.settings.format=="combined")
