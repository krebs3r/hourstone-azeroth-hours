local _, H = ...
local de = GetLocale() == "deDE"
H.de = de
local en = {
    tagline = "Your characters. Your time.", total = "Total time played", characters = "Characters",
    progress = "Progress", keystone = "Keystone", thisWeek = "This week", vault = "Great Vault",
    dungeon = "Dungeons", raid = "Raid", world = "World", slot = "Slot %d", completed = "completed",
    dungeonUnits = "dungeons", raidUnits = "bosses", worldUnits = "activities",
    unknownDungeon = "Unknown dungeon",
    noKeystone = "No keystone", confirmed = "Confirmed", notRecorded = "Not yet recorded", noProgressData = "No data recorded",
    stale = "Outdated", previousWeek = "Previous week", progressUnavailable = "Retail progress unavailable",
    progressLocal = "%d / %d characters · last recorded progress", progressHint = "Each character records progress when played. The optional Companion synchronizes saved progress between selected installations.",
    recordedAt = "Last recorded: %s", bestThisWeek = "Best level this week: %s", noRuns = "No completed dungeons", difficulty = "Difficulty: %s",
    session = "Current session", allTracked = "All tracked characters", character = "Character",
    level = "Level", realm = "Realm", played = "Time played", updated = "Updated",
    guild = "Guild", noGuild = "No guild", guildUnknown = "Not yet recorded",
    allRealms = "All realms", search = "Search character / guild …", combined = "Days + hrs", hours = "Hours",
    searchHelp = "Search the recorded character and guild names. Combine the search with realm and client filters.",
    unavailable = "Not yet available", daysUnit = "d", hoursUnit = "hrs", minutesUnit = "min",
    now = "Just now", minuteAgo = "%d min ago", hourAgo = "%d hr ago", dayAgo = "%d days ago", yesterday = "Yesterday",
    shown = "%d / %d characters shown", filteredTime = "Filtered: %s", oneRealm = "1 realm",
    visible = "%d / %d characters · Visible: %s", partial = " (%d without playtime)",
    hint = "Other characters appear after their first login with Hourstone.",
    name = "Name", noCharacters = "No characters tracked yet.", empty = "No matching characters.", settings = "Settings", minimap = "Show minimap button",
    on = "On", off = "Off", scale = "Hourstone size", reset = "Reset position", done = "Done",
    realms = "%d realms", allClients = "All clients", client = "Client", unknownClient = "Unknown client",
    retail = "Retail", mists = "Mists Classic", tbc = "TBC Anniversary", era = "Classic Era",
    minimapHidden = "Hourstone: minimap button hidden. Open Hourstone with /hourstone or /azerothhours; show the button again in the settings or with /hourstone minimap.",
    minimapHiddenCompartment = "Hourstone: minimap button hidden. Open Hourstone from the Addons menu below the clock or with /hourstone; show the button again in the settings or with /hourstone minimap.",
    open = "Left click: open / close", move = "Drag: move minimap button", commands = "/hourstone or /azerothhours",
    estimate = "Includes locally estimated time since the last confirmed server value.",
    sync = "Last server sync: %s", noSync = "No confirmed /played value has been recorded yet.",
    fullTime = "Total: %s",
    request = "Sync requested", requestWait = "Sync queued (60-second request interval)",
    removedCharacters = "Deleted characters", showTracked = "Show tracked characters", remove = "Delete", restore = "Restore", cancel = "Cancel",
    removeTitle = "Delete character from overview?", removeBody = "%s and its playtime stay saved, but no longer count toward the overview. Your WoW character is not deleted. Restore the entry here or log in again after the deletion has synchronized.",
    removeHint = "Right click: delete from overview", restoreHint = "Right click: restore to overview",
    removedShown = "%d / %d deleted characters shown", noRemoved = "No deleted characters.",
    removalHelp = "Playtime stays saved. A new character login restores known deletions from the overview; /reload does not. For a deletion while offline, synchronize first, then log in again.",
    visibilityError = "Hourstone could not save this change. Your existing character data is unchanged.",
    invalidDB = "Hourstone: saved character controls are invalid. Saved data has been kept; please restore a valid backup.",
    futureDB = "Hourstone: saved data is from a newer addon version. Please update Hourstone.",
}
local german = {
    tagline = "Deine Charaktere. Deine Spielzeit.", total = "Gesamte Spielzeit", characters = "Charaktere",
    progress = "Fortschritt", keystone = "Schlüsselstein", thisWeek = "Diese Woche", vault = "Schatzkammer",
    dungeon = "Dungeon", raid = "Raid", world = "Welt", slot = "Slot %d", completed = "abgeschlossen",
    dungeonUnits = "Dungeons", raidUnits = "Bosse", worldUnits = "Aktivitäten",
    unknownDungeon = "Unbekannter Dungeon",
    noKeystone = "Kein Schlüsselstein", confirmed = "Bestätigt", notRecorded = "Noch nicht erfasst", noProgressData = "Keine Daten vorhanden",
    stale = "Veraltet", previousWeek = "Vorwoche", progressUnavailable = "Retail-Fortschritt nicht verfügbar",
    progressLocal = "%d / %d Charaktere · zuletzt erfasster Fortschritt", progressHint = "Jeder Charakter erfasst seinen Fortschritt beim Spielen. Der optionale Companion gleicht gespeicherten Fortschritt zwischen ausgewählten Installationen ab.",
    recordedAt = "Zuletzt erfasst: %s", bestThisWeek = "Beste Stufe diese Woche: %s", noRuns = "Keine abgeschlossenen Dungeons", difficulty = "Schwierigkeit: %s",
    session = "Aktuelle Sitzung", allTracked = "Alle erfassten Charaktere", character = "Charakter",
    level = "Level", realm = "Realm", played = "Spielzeit", updated = "Aktualisiert",
    guild = "Gilde", noGuild = "Keine Gilde", guildUnknown = "Noch nicht erfasst",
    allRealms = "Alle Realms", search = "Charakter / Gilde suchen …", combined = "Tage + Std.", hours = "Stunden",
    searchHelp = "Suche in den erfassten Charakter- und Gildennamen. Die Suche lässt sich mit Realm- und Clientfiltern kombinieren.",
    unavailable = "Noch nicht verfügbar", daysUnit = "T.", hoursUnit = "Std.", minutesUnit = "Min.",
    now = "Gerade eben", minuteAgo = "vor %d Min.", hourAgo = "vor %d Std.", dayAgo = "Vor %d Tagen", yesterday = "Gestern",
    shown = "%d / %d Charaktere angezeigt", filteredTime = "Gefiltert: %s", oneRealm = "1 Realm",
    visible = "%d / %d Charaktere · Sichtbar: %s", partial = " (%d ohne Spielzeit)",
    hint = "Weitere Charaktere erscheinen nach dem ersten Login mit Hourstone.",
    name = "Name", noCharacters = "Noch keine Charaktere erfasst.", empty = "Keine passenden Charaktere.", settings = "Einstellungen", minimap = "Minimap-Button anzeigen",
    on = "An", off = "Aus", scale = "Hourstone-Größe", reset = "Position zurücksetzen", done = "Fertig",
    realms = "%d Realms", allClients = "Alle Clients", client = "Client", unknownClient = "Unbekannter Client",
    retail = "Retail", mists = "Mists Classic", tbc = "TBC Anniversary", era = "Classic Era",
    minimapHidden = "Hourstone: Minimap-Button ausgeblendet. Hourstone mit /hourstone oder /azerothhours öffnen; den Button in den Einstellungen oder mit /hourstone minimap wieder einblenden.",
    minimapHiddenCompartment = "Hourstone: Minimap-Button ausgeblendet. Hourstone über das Addons-Menü unter der Uhr oder mit /hourstone öffnen; den Button in den Einstellungen oder mit /hourstone minimap wieder einblenden.",
    open = "Linksklick: öffnen / schließen", move = "Ziehen: Minimap-Button verschieben", commands = "/hourstone oder /azerothhours",
    estimate = "Enthält lokal geschätzte Spielzeit seit dem letzten bestätigten Serverwert.",
    sync = "Letzter Serverabgleich: %s", noSync = "Noch kein bestätigter /played-Wert erfasst.",
    fullTime = "Gesamt: %s",
    request = "Abgleich angefragt", requestWait = "Abgleich vorgemerkt (60 Sekunden Anfrageabstand)",
    removedCharacters = "Gelöschte Charaktere", showTracked = "Erfasste Charaktere anzeigen", remove = "Löschen", restore = "Wiederherstellen", cancel = "Abbrechen",
    removeTitle = "Charakter aus der Übersicht löschen?", removeBody = "%s und seine Spielzeit bleiben gespeichert, zählen aber nicht mehr zur Übersicht. Dein WoW-Charakter wird nicht gelöscht. Stelle den Eintrag hier wieder her oder logge dich nach dem Abgleich erneut ein.",
    removeHint = "Rechtsklick: aus der Übersicht löschen", restoreHint = "Rechtsklick: wiederherstellen",
    removedShown = "%d / %d gelöschte Charaktere angezeigt", noRemoved = "Keine gelöschten Charaktere.",
    removalHelp = "Die Spielzeit bleibt gespeichert. Ein neuer Charakter-Login stellt bekannte Löschungen aus der Übersicht wieder her; /reload genügt nicht. Bei einer Löschung während des Offline-Betriebs zuerst synchronisieren, dann neu einloggen.",
    visibilityError = "Hourstone konnte die Änderung nicht speichern. Deine bisherigen Charakterdaten bleiben erhalten.",
    invalidDB = "Hourstone: Die gespeicherten Charaktermarkierungen sind ungültig. Die Daten bleiben erhalten; bitte eine gültige Sicherung wiederherstellen.",
    futureDB = "Hourstone: Die gespeicherten Daten stammen aus einer neueren Addon-Version. Bitte Hourstone aktualisieren.",
}
H.L = de and setmetatable(german, { __index = en }) or en
