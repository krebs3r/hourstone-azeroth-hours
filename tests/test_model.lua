...
assert(db.version==3 and db.settings.scale==2 and db.settings.format=="combined")
assert(db.settings.view=="played")
assert(M.Scale(nil)==1 and M.Scale(0)==.65 and M.Scale(math.huge)==1)
assert(math.abs(M.Scale(1.73)-1.75)<.0001 and M.Scale(2.3)==2)
assert(M.Init({settings={view="progress"}}).settings.view=="progress")
